...
import math
from modules.wav2vec import Wav2Vec2Model
...
class PeriodicPositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, period=25, max_seq_len=1200):
...
class Faceformer(nn.Module):
    def __init__(self, args):
        ...
        # motion encoder  
        self.vertice_map = nn.Linear(args['vertice_dim'], args['feature_dim'])  # 64 for vocaset; 128 for BIWI
        # periodic positional encoding 
        self.PPE = PeriodicPositionalEncoding(args['feature_dim'], period = args['period'])
        # temporal bias
        self.biased_mask = init_biased_mask(n_head = 4, max_seq_len = 1200, period=args['period'])
    ...
    def forward(self, audio, template, vertice, one_hot, criterion,teacher_forcing=True):
        ...
        hidden_states = self.audio_encoder(audio, self.dataset, self.fps,frame_num=frame_num).last_hidden_state
    ...
    def predict(self, audio, template, one_hot):
        template = template.unsqueeze(1) 
        obj_embedding = self.obj_vector(one_hot)
        hidden_states = self.audio_encoder(audio, self.dataset, self.fps).last_hidden_state
